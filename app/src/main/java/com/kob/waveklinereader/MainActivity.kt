...
class MainActivity : AppCompatActivity(), SerialInputOutputManager.Listener {
        ...
        override fun onReceive(context: Context, intent: Intent) {
            when (intent.action) {
                UsbManager.ACTION_USB_DEVICE_ATTACHED -> {
                    if (port == null) {
                        log("ตรวจพบสาย USB เสียบเข้ามา — ลองเชื่อมต่ออัตโนมัติ")
                        connect()
                    }
                }
                UsbManager.ACTION_USB_DEVICE_DETACHED -> {
                    if (port != null) {
                        log("สาย USB ถูกถอดออก — ตัดการเชื่อมต่ออัตโนมัติ")
                        disconnect()
                    }
                }
            }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Auto-detect: react to the cable being physically plugged/unplugged
        // while the app is running, instead of requiring the user to press
        // "เชื่อมต่อสาย" manually every time.
        val usbStateFilter = IntentFilter().apply {
            addAction(UsbManager.ACTION_USB_DEVICE_ATTACHED)
            addAction(UsbManager.ACTION_USB_DEVICE_DETACHED)
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            registerReceiver(usbAttachDetachReceiver, usbStateFilter, Context.RECEIVER_NOT_EXPORTED)
        } else {
            @Suppress("UnspecifiedRegisterReceiverFlag")
            registerReceiver(usbAttachDetachReceiver, usbStateFilter)
        }
        ...
        // Auto-connect on launch in two cases: (1) the app was opened
        // because Android's USB_DEVICE_ATTACHED activity filter matched the
        // FTDI adapter being plugged in just now (device_filter.xml), or
        // (2) the app was opened normally but the adapter was already
        // plugged in beforehand. Either way, the user shouldn't have to
        // press "เชื่อมต่อสาย" manually if the cable is already there.
    ...
    override fun onNewIntent(intent: Intent) {
        ...
        setIntent(intent)
        // Handles the case where the app is already open (foreground or
        // background) and the cable gets plugged in — Android redelivers
        // the USB_DEVICE_ATTACHED intent here instead of creating a new
        // activity instance, since MainActivity is launchMode="singleTop".
        if (intent.action == UsbManager.ACTION_USB_DEVICE_ATTACHED && port == null) {
            connect()
        }
    }
    ...
    override fun onDestroy() {
        ...
        try { unregisterReceiver(usbAttachDetachReceiver) } catch (e: Exception) { /* already unregistered */ }
    ...
    private fun openDevice(device: UsbDevice) {
        ...
        try {
            ...
            // Auto-detect ECU: as soon as the cable/adapter connects
            // successfully, automatically try reading the ECU (reusing the
            // same multi-attempt retry logic as the "เริ่มอ่านซ้ำ" button)
            // instead of requiring a separate manual "ลองอ่าน ECU" press.
            log("เชื่อมต่อสายสำเร็จ — กำลังลองอ่าน ECU อัตโนมัติ")
            retryReadEcu()
        } catch (e: Exception) {
    ...
    // O2 (AF) raw byte -> voltage -> estimated AFR, per formula supplied
    // 2026-09-13. Narrowband O2 sensor: not linear, so voltage->AFR uses a
    // piecewise-linear lookup table rather than a single formula. Voltage
    // is clamped to 0.05-1.00V (the table's own range) before lookup.
    private val o2Table = listOf(
        0.05 to 17.5,
        0.10 to 16.5,
        0.20 to 15.7,
        0.30 to 15.2,
    ...
    private fun rawToO2Voltage(raw: Int): Double {
        val v = (raw and 0xFF) * 5.0 / 255.0
        return round(v, 3)
    }

    private fun voltageToAfr(voltageIn: Double): Double {
        val vMax = o2Table.last().first
        if (voltageIn > vMax) {
            // Voltage above the table's top edge (>1.00V): extrapolate
            // using the slope of the last segment (0.90V->13.3, 1.00V->
            // 12.8) rather than freezing at 12.8, but never report below
            // AFR 10 — that's the floor requested, not a value confirmed
            // by the actual sensor data, so it's a soft limit rather than
            // a real calibrated point. Works out to AFR 10 at ~1.56V.
            val (v0, a0) = o2Table[o2Table.size - 2]
            val (v1, a1) = o2Table[o2Table.size - 1]
            val slope = (a1 - a0) / (v1 - v0)
            val afrRaw = a1 + (voltageIn - v1) * slope
            return round(afrRaw.coerceAtLeast(10.0), 2)
        }
        val v = voltageIn.coerceIn(o2Table.first().first, vMax)
        for (i in 0 until o2Table.size - 1) {
            val (v0, a0) = o2Table[i]
            val (v1, a1) = o2Table[i + 1]
            if (v in v0..v1) {
                val afrRaw = a0 + (v - v0) * (a1 - a0) / (v1 - v0)
                return round(afrRaw, 2)
            }
        }
        return o2Table.last().second
    }

    private fun afrToColor(afr: Double): Int {
        return when {
            afr >= 15.2 -> Color.parseColor("#2196F3") // ลีน - ฟ้า
            afr <= 14.2 -> Color.parseColor("#F44336") // ริช - แดง
            else -> Color.parseColor("#4CAF50")        // ใกล้ stoich - เขียว
        }
    }
    ...
    private fun writeBytes(bytes: ByteArray) {
        ...
        // Skip verbose per-byte hex logging while real-time mode is running —
        // at 80-150ms intervals this was the root cause of the UI freeze
        // (see log()'s comment). Kept in full for single-shot/retry/scan reads.
        if (!isRealtimeRunning) {
    ...
    private fun parseAndDisplay(bytes: ByteArray) {
        ...
        val afRaw = byteAt(offAf)
        val afVoltage = afRaw?.let { rawToO2Voltage(it) }
        val afr = afVoltage?.let { voltageToAfr(it) }
        ...
        mainHandler.post {
            ...
            if (afr != null) {
                tvAf.text = String.format("%.2f", afr)
                tvAf.setTextColor(afrToColor(afr))
            } else {
                tvAf.text = "--"
                tvAf.setTextColor(Color.parseColor("#E8EAED")) // default tvAf color from layout
            }
            tvLight.text = when {
    ...
    // Called once per app launch. Confirms a pending update actually
    // installed by checking whether the running versionCode has increased
    // since last launch — only then is it safe to mark the pending
    // release's timestamp as "seen" (see the comment in downloadAndInstall
    // for why this can't be done at install-launch time). If the user
    // cancelled the install, versionCode stays the same and the pending
    // update remains flagged, so checkForUpdate() will correctly keep
    // offering it again instead of claiming the app is already current.
    private fun confirmUpdateAppliedIfVersionBumped() {
        val lastKnownVersionCode = prefs.getInt("lastKnownVersionCode", -1)
        val currentVersionCode = BuildConfig.VERSION_CODE
        if (lastKnownVersionCode != -1 && currentVersionCode > lastKnownVersionCode) {
            val pending = prefs.getString("pendingReleaseTime", "")
            if (!pending.isNullOrEmpty()) {
                prefs.edit().putString("lastSeenReleaseTime", pending).apply()
            }
        }
        prefs.edit().putInt("lastKnownVersionCode", currentVersionCode).apply()
    }
    ...
    private fun downloadAndInstall(apkUrl: String) {
        ...
        val onComplete = object : BroadcastReceiver() {
            override fun onReceive(context: Context, intent: Intent) {
                ...
                // NOTE: lastSeenReleaseTime is deliberately NOT set here.
                // Launching the installer intent doesn't mean the install
                // actually succeeded — if the user backs out of the system
                // install screen, the app keeps running its OLD version, but
                // this code path still runs. Marking the release "seen" here
                // caused a bug where a cancelled install made the app claim
                // "already on the latest version" while still on the old
                // one. Instead, confirmUpdateAppliedIfVersionBumped() (called
                // from onCreate) marks it seen only once we can confirm from
                // BuildConfig.VERSION_CODE that the install actually took
                // effect on a subsequent launch.
    ...
    // Hard cap on the on-screen log text length. Without this, tvLog (a
    // wrap_content TextView inside a fixed-height ScrollView) grows
    // unbounded during long real-time sessions, and each append()'s forced
    // re-layout of the *entire* log text gets progressively slower until
    // the UI thread stalls — this was the actual root cause of the
    // real-time-mode screen freeze, not the TX/RX logging volume alone.
    private val maxLogChars = 6000
    ...
    private fun log(msg: String) {
        mainHandler.post {
            ...
            tvLog.append("\n$msg")
            if (tvLog.length() > maxLogChars) {
                tvLog.text = tvLog.text.subSequence(tvLog.length() - maxLogChars, tvLog.length())
            }
            if (wasNearBottom) {
